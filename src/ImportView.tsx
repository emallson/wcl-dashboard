...
import { VizState, AppState, importViz, isVizState, CANCEL_IMPORT } from './store';
...
class ImportView extends React.Component<ImportViewProps, ImportViewState> {
    ...
    render() {
        ...
        const textboxStyle = (validText === false) ? {
        ...
        return (
            <>
                ...
                <div id="import-view">
                    <textarea placeholder="Paste string here..." cols={80} rows={8} 
                        onChange={(e) => this.validateText(e.target.value)}
                        style={textboxStyle} /><br/>
                    <button style={{'float': 'left'}} onClick={cancelImport}>Cancel</button>
                    <button onClick={() => currentState ? importViz(currentState) : null}
                        disabled={currentState === null}>Import</button>
                </div>
            </>
        );
    }
