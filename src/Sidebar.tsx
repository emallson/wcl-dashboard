...
const _MenuBar: React.FC<MenuBarProps> = ({create, beginImport, code, update, title}) => {
    ...
    return (
        <>
            <div className="menu-top">
                <Icon icon={menu_icon} onClick={() => setSidebarVisible(!sidebarVisible)} />
                <span>{title ? title : 'No Log Selected'}</span>
                <Icon icon={refresh} style={{
                    marginLeft: '0.5em',
                }} size={16} onClick={() => code ? update(code) : null}/>
                <div className="right">
...
const ReportEntry: React.FC<ReportEntryProps> = (props) => {
    ...
    return (
        <div  className={`sidebar-item ${context.selected ? 'selected' : ''}`}>
            <em>{props.date.toLocaleString('en-US', {
                weekday: 'short',
                month: 'short',
                day: 'numeric',
                year: 'numeric',
...
const _SideBar: React.FC<SideBarProps> = (props) => {
    ...
    return (
        <div className='menu-side'>
            <SideNav
                onSelection={(code) => { props.closeSidebar(); props.setMainReport(code); }}
                defaultSelectedPath={props.currentCode ? props.currentCode.toString() : ''}>
                {reportItems}
            </SideNav>
            <ReportInput 
...
function sidebarState(state: AppState) {
    return {
        ...
        reports: state.reports.map((report, code) => {
            return {
                code,
                date: new Date(report.start!),
                title: report.title!,
            };
        }).valueSeq().sortBy(r => r.date).reverse().toArray(),
    };
}

function sidebarDispatch(dispatch: Dispatch) {
    return {
        updateReport: (code: string) => dispatch<any>(updateReport(toReportCode(code))),
        setMainReport: (code: string) => dispatch(setMainReport(toReportCode(code))),
    };
}
