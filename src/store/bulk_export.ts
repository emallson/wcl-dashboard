...
export function reducer(state = initialVizList, action: DashboardAction): VizList {
    if(action.type === BULK_EXPORT) {
        const tsv = state.map((state) => `${state.spec.title}\t${export_view(state)}`).valueSeq().toArray().join("\n");
        saveAs(new Blob([tsv], {type: 'text/tab-separated-values;charset=utf-8'}), "visualizations.tsv");
    }
    return state;
}
