...
describe('Component: QueryBuilder', () => {
    ...
    it("should display a 'Unknown (bossid)' for queries whose boss is not in the bosses list", () => {
        ...
        const bosses = Map().set(0, 'a').set(1, 'b').set(2, 'c');
        const { getByText } = render(<QueryBuilder guid={createGuid()} meta={meta} bosses={bosses} setQueryViz={() => {}}/>);
    ...
    it("should not display a 'Unknown (bossid)' for queries whose boss is in the bosses list", () => {
        ...
        const bosses = Map().set(0, 'a').set(1, 'b').set(2, 'c');
        const { queryByText } = render(<QueryBuilder guid={createGuid()} meta={meta} bosses={bosses} setQueryViz={() => {}}/>);
