...
const QueryBuilder: React.FC<QueryBuilderProps> = ({ guid, meta, setVizQuery, bosses }) => {
    ...
    const bossSelector = meta ? (
        <select defaultValue={meta.bossid || 'null'} onChange={(e) => {
            setVizQuery(guid, meta.kind.kind, tableKind(meta), meta.filter, (e.target.value === 'null') ? null : e.target.value);
        }}>
    ...
    const tableSelector = (meta && meta.kind.kind === QueryType.Table) ? (
        <select defaultValue={meta.kind.table} onChange={(e) => {
                setVizQuery(guid, meta.kind.kind, e.target.value, meta.filter, meta.bossid);
            }}>
    ...
    const filterEditor = meta ? (
        <input type="text" defaultValue={meta.filter} 
            onBlur={(e) => setVizQuery(guid, meta.kind.kind, tableKind(meta), e.target.value, meta.bossid)}
        />
            ) : null;
    return (
            ...
            <select defaultValue={meta ? meta.kind.kind : DEFAULT_VALUE } onChange={(e) => {
                switch(e.target.value) {
                    case QueryType.Event:
                        setVizQuery(guid, e.target.value, null, meta ? meta.filter : '', meta ? meta.bossid : null);
                        break;
                    case QueryType.Table:
                        // can't have already been table, default to
                        // damage
                        setVizQuery(guid, e.target.value, TableKind.Damage, 
                            meta ? meta.filter : '', meta ? meta.bossid : null);
                        break;
                    default:
...
const mapDispatch = (dispatch: Dispatch) => {
        return {
            setVizQuery: (guid: Guid, kind: string, table: string | null, filter: string, bossid: string | null) => dispatch(setVizQuery(guid, kind, table, filter, bossid)),
        };
};
